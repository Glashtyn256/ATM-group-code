...
public:
	...
	// credits to helios (14105) on http://www.cplusplus.com big up helios
	bool isLeapYear(unsigned short year) const;
	bool isValid(const Date& _date) const;
	//---------------------------
	...
	bool operator>(const Date& d) const;	//true if (*this > d)  (strictly later)
	bool operator<=(const Date& d) const;	//true if (*this <= d)  (strictly earlier or the same as)
	bool operator>=(const Date& d) const;	//true if (*this >= d)  (strictly later or the same as)
